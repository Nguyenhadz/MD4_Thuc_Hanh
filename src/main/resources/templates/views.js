...
function showFormAdd() {
    document.getElementById('showsEmployee').innerHTML = '';
    document.getElementById('findByDepartment').innerHTML = '';
    document.getElementById('showListByDepartment').innerHTML = '';
    let str =
        `
...
function showFormFindByDepartment() {
    axios.get(API_DEPARTMENT).then((res)=>{
        ...
        <h2><b>Find By Department</b></h2>
            <select style="width: 300px;" id="departmentFind" class="form-control">
            `
        for (let i = 0; i < list.length; i++) {
            str +=
                `
        ...
        <div>
            <button class="btn btn-primary" onclick="findDepartment()">Find</button>
        </div>

            `
        document.getElementById('findByDepartment').innerHTML = str
    })
}
function findDepartment() {
    let data = {
        id: document.getElementById('departmentFind').value
    }
    axios.get(API_EMPLOYEE + '/department/' + data.id).then((res)=>{
        let list = res.data
        document.getElementById("showListByDepartment").innerHTML = ''
        let str = `<table class="table table-striped">
                            <thead>
        ...
        document.getElementById("showListByDepartment").innerHTML = str
    })
}